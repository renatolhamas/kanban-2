...
- **Buttons:**
  - _Primary:_ Emerald gradient background, white text, 8px radius.
  - _Secondary:_ `secondary-container` (#d2e0fe) background with `on-secondary-container` (#55637d) text. No border.
- **Input Fields:** Use `surface-container-lowest` (#ffffff) for the fill. The label should be `label-md` in `on-surface-variant` (#3c4a42). Use a 2px `primary` bottom-border _only_ on focus.
