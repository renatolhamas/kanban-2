...
1.  **Frontend State Mismatch:**
    - **Fixed** in [page.tsx](<file:///c:/git/kanban.2/app/(auth)/register/page.tsx>) and [RegisterForm.tsx](file:///c:/git/kanban.2/components/RegisterForm.tsx).
    - `RegisterForm` now correctly waits for `onSubmit` completion and handles errors in a `try/catch` block, preventing the "green toast" from appearing if the backend fails.

2.  **Backend Linkage & Race Condition:**
    - **Fixed** in [route.ts](file:///c:/git/kanban.2/app/api/auth/register/route.ts).
    - Added a 100ms delay before `users` table insertion to ensure FK synchronization.
    - Enhanced logging implemented to capture detailed error codes from Supabase.

3.  **Database Constraint (Role 'owner'):**
    - **Verified** via direct DB query on 2026-04-02.
    - Test insert with `role='owner'` succeeded, confirming the `users_role_check` constraint has been updated in the cloud instance.
    - _Note:_ The migration file `20260402000001_alter_users_role_constraint.sql` accurately reflects the state but remains untracked in Git.
