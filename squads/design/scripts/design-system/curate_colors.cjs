...
function classifyColor(oklch, hex) {
  ...
  if (C < 0.02) {
    ...
    if (L > 0.7) return "gray-200";
    if (L > 0.55) return "gray-300";
    if (L > 0.4) return "gray-400";
...
function generateTonalScale(oklch) {
  ...
  return steps.map(({ step, L }) => {
    ...
    const scaledC =
      C * Math.min(1, (1 - Math.abs(L - 0.55) / 0.55) * 1.5 + 0.3);
    return {
      step,
...
function main() {
  ...
  try {
    figmaVariables = JSON.parse(fs.readFileSync(VARIABLES_PATH, "utf8"));
  } catch (_) {
    /* optional */
  }
  ...
  console.log(
    `  Figma color styles: ${colorStyles.length}, Figma color variables: ${colorVariables.length}`,
  );

  console.log(
    `Input: ${rawColors.length} unique colors from ${merged.meta.pagesIncluded} pages`,
  );
  ...
  const curatedPalette = clusters
    ...
    .map((cluster) => {
      ...
      const matchedStyle = colorStyles.find(
        (s) =>
          s.name &&
          representative.contexts.some(
            (ctx) =>
              ctx &&
              s.name.toLowerCase().includes(ctx.toLowerCase().split(":")[0]),
          ),
      );
      ...
      const matchedVariable = colorVariables.find((v) => {
        ...
        for (const modeValues of Object.values(v.valuesByMode)) {
          if (modeValues && modeValues.r !== undefined) {
            const vHex =
              "#" +
              [modeValues.r, modeValues.g, modeValues.b]
                .map((c) =>
                  Math.round(c * 255)
                    .toString(16)
                    .padStart(2, "0"),
                ...
                .join("");
            if (vHex.toLowerCase() === representative.hex.toLowerCase())
              return true;
          }
        }
        return false;
  ...
  const chromaticGroup = curatedPalette.filter(
    (c) => !achromaticGroup.includes(c),
  );
  ...
  const reduction =
    ((rawColors.length - curatedPalette.length) / rawColors.length) * 100;
  ...
  const scores = {
    ...
    coverage_status:
      coverage >= 95 ? "PASS" : coverage >= 85 ? "CONDITIONAL" : "FAIL",
  };
