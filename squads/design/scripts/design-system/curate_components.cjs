...
const ATOM_PATTERNS =
  /\b(button|btn|input|label|badge|avatar|icon|tag|chip|switch|toggle|checkbox|radio|divider|spinner|loader|skeleton)\b/i;
const MOLECULE_PATTERNS =
  /\b(form[-_]?field|card|alert|tooltip|toast|dropdown|select|menu[-_]?item|tab|breadcrumb|pagination|search|popover|dialog|modal[-_]?header|accordion)\b/i;
const ORGANISM_PATTERNS =
  /\b(form|modal|sidebar|navbar|header|footer|table|data[-_]?table|calendar|carousel|wizard|stepper|layout|page|section|panel|drawer)\b/i;
...
const P1_PATTERNS =
  /\b(badge|avatar|form|modal|alert|select|dropdown|toast|dialog)\b/i;
...
const VARIANT_PATTERNS = {
  state:
    /\b(default|hover|active|focus|disabled|pressed|loading|error|success|selected)\b/i,
  size: /\b(xs|sm|md|lg|xl|2xl|small|medium|large|mini|compact)\b/i,
  variant:
    /\b(primary|secondary|tertiary|ghost|outline|link|destructive|warning|info)\b/i,
};
...
function main() {
  ...
  console.log(
    `Input: ${rawComponents.length} unique components from ${merged.meta.pagesIncluded} pages`,
  );
  ...
  console.log(
    `  After frequency filter (>= ${MIN_INSTANCES}): ${filtered.length}`,
  );
  ...
  const curated = filtered.map((comp) => {
    ...
    const nameForClassification =
      comp.name || (comp.sizes ? comp.sizes.join(" ") : "");
    ...
    if (atomicLevel === "unknown") {
      ...
      const propCount = comp.componentProperties
        ? Object.keys(comp.componentProperties).length
        : 0;
      if (propCount >= 3) {
    ...
    const variants = detectVariants(nameForClassification, comp.contexts || []);
    ...
    return {
      ...
      variants,
      hasComponentProperties: comp.componentProperties
        ? Object.keys(comp.componentProperties).length > 0
        : false,
      pages: comp.pages || [],
    };
  });
  ...
  console.log(
    `  Atomic: ${atoms.length} atoms, ${molecules.length} molecules, ${organisms.length} organisms, ${unknown.length} unknown`,
  );
  console.log(`  Priority: ${p0.length} P0, ${p1.length} P1, ${p2.length} P2`);
  console.log(`\n  Top 5 most-used:`);
  for (const c of curated.slice(0, 5)) {
    console.log(
      `    ${c.name || "(unnamed)"} [${(c.sizes || []).join(", ")}] — ${c.count}x (${c.atomicLevel}, ${c.priority})`,
    );
  }
  console.log(`\nOutput: ${OUTPUT_PATH}`);
  console.log("=== DONE ===");
