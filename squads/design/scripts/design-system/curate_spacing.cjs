...
function main() {
  ...
  console.log(
    `Input: ${rawSpacings.length} unique spacing values from ${merged.meta.pagesIncluded} pages`,
  );
  ...
  if (detectedGcd !== 4 && detectedGcd !== 8 && detectedGcd !== 2) {
    ...
    gcdStatus = "override";
    console.log(
      `  GCD detected: ${detectedGcd}px → Overriding to 4px (market standard)`,
    );
  } else {
  ...
  for (const val of uniqueValues) {
    ...
    if (distance <= MAX_DISTANCE) {
      ...
      const acc = usedSteps.get(step.name) || {
        step: step.name,
        px: step.px,
        count: 0,
      };
  ...
  const reduction =
    ((uniqueValues.length - finalScale.length) / uniqueValues.length) * 100;
