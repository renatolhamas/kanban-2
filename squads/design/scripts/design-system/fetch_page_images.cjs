...
function figmaGet(endpoint) {
  return new Promise((resolve, reject) => {
    ...
    https
      .get(
        url,
        { headers: { "X-Figma-Token": TOKEN }, timeout: 60000 },
        (res) => {
          ...
          res.on("end", () => {
            try {
              ...
              if (res.statusCode >= 400) {
                reject(
                  new Error(
                    `API ${res.statusCode}: ${body.err || body.message || "unknown"}`,
                  ),
                );
              } else {
                resolve(body);
...
async function main() {
  ...
  const sortedPages = [...contentPages].sort(
    (a, b) => (a.stats?.totalNodes || 0) - (b.stats?.totalNodes || 0),
  );
  ...
  for (let i = 0; i < sortedPages.length; i++) {
    ...
    if (fs.existsSync(destPath) && fs.statSync(destPath).size > 100) {
      console.log(
        `[${i + 1}/${sortedPages.length}] ✓ ${page.slug}: already exists`,
      );
      fetched++;
      continue;
    }
    ...
    console.log(
      `[${i + 1}/${sortedPages.length}] ${page.slug} (${nodes} nodes, scale=${scale})...`,
    );
    ...
    while (!success && retries <= maxRetries) {
      try {
        const result = await figmaGet(
          `/images/${FILE_ID}?ids=${encodeURIComponent(page.nodeId)}&format=png&scale=${scale}`,
        );
