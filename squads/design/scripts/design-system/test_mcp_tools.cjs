...
const tokenRegistry = readJson(
  "workspace/domains/design-system/metadata/tokens/index.json",
);
const guidelines = readJson(
  "workspace/domains/design-system/metadata/guidelines/usage.json",
);
...
function getTokensByCategory(category) {
  return tokenRegistry.tokens.filter(
    (t) => t.category === category.trim().toLowerCase(),
  );
}

function getTokensByTier(tier) {
  return tokenRegistry.tokens.filter(
    (t) => t.tier === tier.trim().toLowerCase(),
  );
}
...
function callTool(name, args) {
  switch (name) {
    ...
    case "design_system.get_tokens": {
      ...
      if (category && tier) {
        return getTokensByCategory(category).filter(
          (t) => t.tier === tier.trim().toLowerCase(),
        );
      }
...
const tests = [
  ...
  },
  {
    ...
    args: { id: "button" },
    validate: (r) =>
      r !== null && r.id === "button" && Array.isArray(r.variants),
  },
  {
  ...
  },
  {
    ...
    validate: (r) =>
      Array.isArray(r) &&
      r.length >= 20 &&
      r.every((t) => t.category === "color"),
  },
  {
    tool: "design_system.get_guidelines",
    args: { component: "button" },
    validate: (r) =>
      r !== null && Array.isArray(r.when_to_use) && r.when_to_use.length > 0,
  },
  {
