...
const GATES = [
  {
    ...
    check: (r) => {
      ...
      if (pct >= 85)
        return { status: "PASS", value: r.scores.colors.reduction };
      if (pct >= 70)
        return { status: "CONDITIONAL", value: r.scores.colors.reduction };
  ...
  },
  {
    ...
    check: (r) => {
      ...
      if (pct >= 100)
        return { status: "PASS", value: r.scores.colors.wcag_aa_pass };
      if (pct >= 90)
        return { status: "CONDITIONAL", value: r.scores.colors.wcag_aa_pass };
  ...
  },
  {
    ...
    check: (r) => {
      ...
      if (pct >= 60)
        return { status: "PASS", value: r.scores.spacing.reduction };
      if (pct >= 40)
        return { status: "CONDITIONAL", value: r.scores.spacing.reduction };
  ...
  },
  {
    ...
    check: (r) => {
      ...
      if (pct >= 95)
        return { status: "PASS", value: r.scores.spacing.coverage };
      if (pct >= 85)
        return { status: "CONDITIONAL", value: r.scores.spacing.coverage };
  ...
  },
  {
    ...
    check: (r) => {
      ...
      if (pct >= 50)
        return { status: "PASS", value: r.scores.typography.reduction };
      if (pct >= 35)
        return { status: "CONDITIONAL", value: r.scores.typography.reduction };
  ...
  },
  {
    ...
    check: (r) => {
      ...
      if (count === 4)
        return { status: "CONDITIONAL", value: `${count} families` };
  ...
  },
  {
    ...
    check: (r) => {
      ...
      if (pct >= 70)
        return { status: "PASS", value: r.scores.shadows.reduction };
      if (pct >= 50)
        return { status: "CONDITIONAL", value: r.scores.shadows.reduction };
  ...
  },
  {
    ...
    check: (r) => {
      ...
      if (pct >= 60)
        return { status: "PASS", value: r.scores.radius.reduction };
      if (pct >= 40)
        return { status: "CONDITIONAL", value: r.scores.radius.reduction };
  ...
  },
  {
    ...
    check: (r) => {
      ...
      if (pct >= 80)
        return { status: "PASS", value: r.scores.overall.reduction };
      if (pct >= 60)
        return { status: "CONDITIONAL", value: r.scores.overall.reduction };
    ...
    check: () => {
      try {
        ...
        if (pct >= 90)
          return {
            status: "PASS",
            value: `${Math.round(pct)}% (${named.length}/${all.length})`,
          };
        if (pct >= 70)
          return {
            status: "CONDITIONAL",
        ...
        return {
          status: "CONDITIONAL",
          value: "curated-components.json not found",
        };
      }
    },
    target: ">= 90%",
  },
  {
    ...
    check: () => {
      try {
        ...
        if (count > 0)
          return { status: "PASS", value: `${count} styles fetched` };
        return {
          status: "CONDITIONAL",
          value: "0 styles (endpoint may not be available)",
        };
      } catch (_) {
    ...
    const icon =
      result.status === "PASS"
        ...
        : result.status === "CONDITIONAL"
          ...
          : "✗";
    console.log(
      `  [${gate.id}] ${icon} ${gate.name}: ${result.value} (target: ${gate.target}) → ${result.status}`,
    );
