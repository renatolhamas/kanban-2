...
function main() {
  ...
  if (!registry.$schema) {
    warn(
      "Missing $schema reference (recommended: https://ui.shadcn.com/schema/registry.json)",
    );
  }
  ...
  for (let idx = 0; idx < registry.items.length; idx++) {
    ...
    if (typeof item.type !== "string" || !item.type) {
      ...
      error(
        `${prefix}: invalid type "${item.type}". Must be one of: ${VALID_ITEM_TYPES.join(", ")}`,
      );
    }
    ...
    if (
      item.devDependencies !== undefined &&
    ...
    if (item.registryDependencies !== undefined) {
      if (!Array.isArray(item.registryDependencies)) {
        ...
        for (const dep of item.registryDependencies) {
          ...
          if (!allNames.has(dep)) {
            error(
              `${prefix}: registryDependency "${dep}" not found in registry`,
            );
          }
        }
      }
    ...
    if (
      ...
      (typeof item.cssVars !== "object" || Array.isArray(item.cssVars))
    ) {
