...
function extractDesignData() {
  ...
  for (let i = 0; i < rootStyles.length; i++) {
    ...
    if (prop.startsWith("--")) {
      result.cssRules.customProperties.set(
        prop,
        rootStyles.getPropertyValue(prop).trim(),
      );
    }
  }
  ...
  function walkDOM(element, depth = 0) {
    ...
    if (styles["color"])
      result.cssRules.colors.set(
        styles["color"],
        (result.cssRules.colors.get(styles["color"]) || 0) + 1,
      );
    if (
      ...
      result.cssRules.colors.set(
        styles["background-color"],
        (result.cssRules.colors.get(styles["background-color"]) || 0) + 1,
      );
    }
    if (
      styles["background-image"] &&
      styles["background-image"].includes("gradient")
    ) {
    ...
    if (styles["font-family"]) {
      ...
      result.cssRules.typography.set(
        key,
        (result.cssRules.typography.get(key) || 0) + 1,
      );
    }
    if (styles["box-shadow"])
      result.cssRules.shadows.set(
        styles["box-shadow"],
        (result.cssRules.shadows.get(styles["box-shadow"]) || 0) + 1,
      );
    if (styles["transition"])
      result.cssRules.transitions.push(styles["transition"]);
    ...
    [
      ...
      if (styles[prop] && styles[prop] !== "0px") {
        result.cssRules.spacing.set(
          styles[prop],
          (result.cssRules.spacing.get(styles[prop]) || 0) + 1,
        );
      }
    });
    ...
    if (styles["border-width"] && styles["border-width"] !== "0px") {
      ...
      result.cssRules.borders.set(
        key,
        (result.cssRules.borders.get(key) || 0) + 1,
      );
    }
  ...
  const componentSelectors = [
    ...
    {
      pattern: 'button, [role="button"], input[type="submit"]',
      type: "button",
    },
    {
      pattern: 'a.btn, a.button, a.cta, a[class*="btn"], a[class*="button"]',
      type: "cta-link",
    },
    { pattern: "input, textarea, select", type: "form-input" },
    { pattern: '[class*="card"], [class*="Card"]', type: "card" },
    {
      pattern: '[class*="modal"], [class*="Modal"], [role="dialog"]',
      type: "modal",
    },
    { pattern: '[class*="hero"], [class*="Hero"]', type: "hero" },
    {
      pattern: '[class*="dropdown"], [class*="Dropdown"], [class*="menu"]',
      type: "dropdown",
    },
    { pattern: '[class*="tab"], [role="tablist"]', type: "tabs" },
    { pattern: '[class*="accordion"]', type: "accordion" },
    {
      pattern: '[class*="toast"], [class*="notification"], [class*="alert"]',
      type: "notification",
    },
    {
      pattern: '[class*="badge"], [class*="tag"], [class*="chip"]',
      type: "badge",
    },
    { pattern: '[class*="avatar"]', type: "avatar" },
    { pattern: '[class*="tooltip"]', type: "tooltip" },
    {
      pattern: 'img, picture, [class*="image"], [class*="Image"]',
      type: "image",
    },
    { pattern: 'video, [class*="video"]', type: "video" },
    {
      pattern: '[class*="slider"], [class*="carousel"], [class*="swiper"]',
      type: "carousel",
    },
...
async function downloadStylesheets(page, outputDir) {
  ...
  for (const { index, cssText } of embeddedStyles) {
    ...
    downloaded.push({
      embedded: true,
      file: `embedded-${index}.css`,
      size: cssText.length,
    });
    console.error(
      `  Extracted: embedded-${index}.css (${cssText.length} bytes)`,
    );
  }
...
async function main() {
  ...
  const outputDir =
    args.output ||
    path.join(projectRoot, "outputs", "design-system", "scans", args.name);
  ...
  try {
    ...
    console.error(
      `  Saved: dom-tree.json (${designData.summary.nodeCount} nodes)`,
    );
    ...
    console.error(
      `  Saved: components.json (${designData.components.length} component types)`,
    );
    ...
    const manifest = {
      ...
      files: {
        ...
        screenshots: [
          ...
          ...(args.mobile ? ["screenshot-mobile.png"] : []),
        ],
        ...
        components: {
          ...
          count: designData.components.length,
        },
        tokens: "tokens.yaml",
      },
      summary: {
        ...
        customProperties: Object.keys(designData.cssRules.customProperties)
          .length,
        ...
        totalComponents: designData.components.reduce(
          (sum, c) => sum + c.count,
          0,
        ),
    ...
    console.error(
      `  Typography:         ${manifest.summary.typographyVariants} variants`,
    );
