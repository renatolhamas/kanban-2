...
function cmdStart(slug, options = {}) {
  ...
  const updateResult = runStateManager(
    `update ${slug} --phase research --status in_progress --force`,
  );
  if (!updateResult.success && updateResult.error) {
    outputError(
      "UPDATE_FAILED",
      "Failed to advance to research phase",
      updateResult.error,
    );
    process.exit(1);
  }
...
function cmdNext(slug, options = {}) {
  ...
  if (currentStatus !== "completed" && currentStatus !== "approved" && !force) {
    outputError(
      "PHASE_NOT_COMPLETE",
      `Current phase "${currentPhase}" is not complete`,
      {
  ...
  if (nextPhase.human_checkpoint) {
    const updateResult = runStateManager(
      `update ${slug} --phase ${nextPhase.id} --status checkpoint`,
    );
    outputJson({
      success: true,
      action: "next",
  ...
  const updateResult = runStateManager(
    `update ${slug} --phase ${nextPhase.id} --status in_progress`,
  );
...
function cmdStatus(slug) {
  ...
  const progress = Math.round(
    (currentPhaseIdx / (PIPELINE_PHASES.length - 1)) * 100,
  );
...
function cmdApprove(slug) {
  ...
  runStateManager(
    `update ${slug} --phase ${state.current_phase} --status approved`,
  );
...
Agents:
  ...
  if (command === "start") {
    ...
    if (!slug) {
      outputError(
        "NO_ACTIVE_SQUAD",
        "No slug provided and no .active-squad file",
      );
      process.exit(1);
    }
    cmdResume(slug);
